...
export default function ProfilePage() {
  ...
  async function save() {
    ...
    const { error } = await createClient().from('profiles')
      .update({ display_name: displayName, bio, is_anonymous: isAnonymous })
      .eq('id', user.id)
    setSaving(false)
      ...
      <div className="pb-8">
        ...
        <div className="mx-4 -mt-8 card p-4 mb-4">
          ...
          <input value={displayName} onChange={e => setDisplayName(e.target.value)}
            className="bg-transparent font-bold text-xl outline-none w-full placeholder-dark-300 text-white" placeholder="Your name" />
          <p className="text-teal-400 text-sm my-1">{profile?.username}@whisper.me</p>
          <textarea value={bio} onChange={e => setBio(e.target.value)} rows={2}
            className="bg-transparent text-sm text-dark-300 outline-none w-full resize-none placeholder-dark-300" placeholder="Short bio…" />
        ...
        <div className="mx-4 card p-4 mb-4">
          ...
          <div className="flex items-center gap-2 bg-dark-700 border border-dark-400 rounded-xl px-3 py-2.5">
            ...
            <button onClick={copyLink} className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition flex-shrink-0 ${copied ? 'bg-green-500 text-white' : 'bg-teal-500 text-white'}`}>
              {copied ? 'Copied!' : 'Copy'}
            </button>
          </div>
        </div>
        ...
        <div className="mx-4 card mb-4 overflow-hidden">
          ...
          <div className="flex items-center justify-between px-4 py-3.5">
            <div>
              ...
              <p className="text-xs text-dark-300 mt-0.5">Hide your real name</p>
            </div>
            <button onClick={() => setIsAnonymous(!isAnonymous)}
              className={`w-11 h-6 rounded-full transition-colors relative flex-shrink-0 ${isAnonymous ? 'bg-teal-500' : 'bg-dark-400'}`}>
              <span className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${isAnonymous ? 'left-[22px]' : 'left-0.5'}`} />
            </button>
          </div>
        </div>
