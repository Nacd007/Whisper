...
const NearbyMap = dynamic(() => import('@/components/discover/NearbyMap'), {
  ssr: false,
  loading: () => (
    <div className="mx-4 h-48 rounded-2xl bg-dark-700 border border-dark-400 flex items-center justify-center text-dark-300 text-sm">
      Loading map…
    </div>
  )
})
...
export default function DiscoverPage() {
  ...
  useEffect(() => {
    getLocation().then(c => fetchNearby(c.lat, c.lng)).catch(() => {})
  }, [])
  ...
  async function fetchNearby(lat: number, lng: number) {
    ...
    if (user) {
      try {
        await supabase.rpc('update_my_location', { user_lat: lat, user_lng: lng })
      } catch { }
    }
  ...
  async function startChat(otherId: string) {
    ...
    if (ids.length) {
      const { data: shared } = await supabase.from('conversation_members')
        .select('conversation_id').eq('user_id', otherId).in('conversation_id', ids).single()
      if (shared) { window.location.href = `/chat/${(shared as any).conversation_id}`; return }
    }
    ...
    await supabase.from('conversation_members').insert([
      ...
      { conversation_id: (conv as any).id, user_id: otherId }
    ])
    window.location.href = `/chat/${(conv as any).id}`
  }
    ...
    <AppShell>
      <div className="px-4 pt-12 pb-4">
        ...
        <p className="text-sm text-dark-300 mt-1">
          {coords ? `${users.length} people within 5km` : 'Enable location to find people nearby'}
        </p>
      </div>
      ...
      <div className="flex gap-2 px-4 pb-4 overflow-x-auto">
        {['All', 'Online', 'Within 1km', 'Music 🎵', 'Art 🎨', 'Gaming 🎮'].map((f, i) => (
          <button key={f} className={`px-4 py-1.5 rounded-full text-xs font-medium border whitespace-nowrap flex-shrink-0 ${i === 0 ? 'border-teal-500 text-teal-300 bg-teal-500/10' : 'border-dark-400 text-dark-300'}`}>
            {f}
          </button>
        ))}
      </div>
      ...
      <div className="px-4 flex flex-col gap-3 pb-6">
        {!coords && !locLoading && (
          <div className="text-center py-10">
            ...
            <button onClick={() => getLocation().then(c => fetchNearby(c.lat, c.lng))}
          ...
          return (
            <div key={u.id} className="card p-4 flex items-center gap-3">
              <div className="relative flex-shrink-0">
                <div className={`w-12 h-12 rounded-full bg-gradient-to-br ${avatarColor(u.username)} flex items-center justify-center font-bold text-white`}>
                  {getInitials(name)}
                </div>
              ...
              <button onClick={() => startChat(u.id)}
                className="px-4 py-2 rounded-xl border border-teal-500 text-teal-400 text-xs font-semibold hover:bg-teal-500 hover:text-white transition active:scale-95 flex-shrink-0">
                Message
              </button>
            </div>
          )
